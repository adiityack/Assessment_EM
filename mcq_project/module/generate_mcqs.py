...
# Set the correct path for the questions.json file in the mcq_project root directory
...
for attempt in range(retries):
    try:
        ...
        if is_valid_output(new_questions):
            ...
            # Save combined questions to the correct path
            with open(QUESTION_FILE_PATH, "w") as f:
                json.dump(all_questions, f, indent=2)
            print(f"New questions appended to {QUESTION_FILE_PATH}")
            break
        else:
            raise ValueError("Invalid question format found")
